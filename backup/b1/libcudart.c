...
__attribute__((constructor)) void init_mocu(){
  ...
  mocu.mocudaGetDeviceCount(&mocu.ndev);

  res = nvmlInit();

  if(res != NVML_SUCCESS){

    printf("Failed to initialize Mobile CUDA with NVML\n");
    ...
    for(i = 0 ; i < mocu.ndev ; i ++){
      ...
      res = nvmlDeviceGetHandleByIndex(i,&mocu.nvml_dev[i]);

      if(res != NVML_SUCCESS){
	printf("Failed to get nvml-device-handle @ %d\n",i);
      }
    }
  }
...
void __cudaUnregisterFatBinary(void** fatCubinHandle){
  ...
  int i,procs = 0;
  for(i = 0 ; i < mocu.ndev ; i ++){
    procs += mocu_num_of_proc_at_device(i);
  }
...
void __cudaRegisterVar(void** fatCubinHandle,char *hostVar,char *deviceAddress,const char *deviceName,int ext,int size,int constant,int global){
  ...
  return mocu.__mocudaRegisterVar(fatCubinHandle,hostVar,deviceAddress,deviceName,ext,size,constant,global);
...
cudaError_t cudaLaunch(const void *func){
  ...
  if(launch_counter++ == 3){
    ...
    size = mocu_num_of_proc_at_device(0);

    printf("\n\nprocs(%d)\n\n",size);

    infos = (nvmlProcessInfo_t*)malloc(sizeof(nvmlProcessInfo_t)*size);
    infos = mocu_get_proc_info_at_device(0);

    int i;
    for(i = 0 ; i < size ; i ++){
      printf("[%d]PID   == %d\n",i,infos[i].pid);
      printf("    USAGE == %u\n",infos[i].usedGpuMemory);

      if(infos[i].pid != getpid()){
	mocu_send_signal(infos[i].pid,MOVE_TO_OTHER_DEVICE);
      }
...
cudaError_t cudaMalloc(void **devPtr,  size_t size){
  ...
  if(res == cudaSuccess){
    ...
    int i;
    int _size = mocu_get_amount_of_memory_used();

    for(i = 0 ; i < mocu.ndev ; i ++){
	
      if(i == mocu_pos)continue;

      mocu_lock();
	
      nvmlMemory_t mem_info = mocu_get_mem_info_at_device(i);
	
      if(mem_info.free > size + _size){
	mocu_migrate(i);
	mocu_unlock();
	break;
      }
	...
	printf("RECOVER FROM BE SUSPENDED!!!!!!!!!!!!!!!!!!!!!!!!!\n");
...
cudaError_t cudaMemcpyToSymbol(const void *symbol,  const void *src,  size_t count,  size_t offset ,  enum cudaMemcpyKind kind ){
  ...
  return mocu.mocudaMemcpyToSymbol(symbol, src, count, offset , kind );
