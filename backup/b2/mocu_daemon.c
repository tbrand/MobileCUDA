	...
	if(i == listen_sd){
	  ...
	  if(rc <= 0){
	    ...
	    if(_proc->REQUEST == CONNECT){
	      ...
	      _proc->pos = 0;
	      _proc->REQUEST = STARTPOS;

	      send(i,_proc,sizeof(proc_data),0);
	      ...
	      recievedData->data = _proc;
	      recievedData->queued = 1;

	    }else if(_proc->REQUEST == CUDAMALLOC){
		...
		exit(-1);
	      ...
	      if(mem.free > _proc->mem-prevRegion){

		printf("\tGoahead!!\n");

		proc_data* goahead = (proc_data*)malloc(sizeof(proc_data));

		goahead->REQUEST = CONNECT;

		send(i,goahead,sizeof(proc_data),0);

	      }else{

		printf("\tOops.. Stop!!\n");

		proc_data* stop = (proc_data*)malloc(sizeof(proc_data));

		stop->REQUEST = SUSPEND;

		send(i,stop,sizeof(proc_data),0);
