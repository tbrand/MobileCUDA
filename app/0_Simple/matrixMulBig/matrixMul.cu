...
int matrixMultiply(int argc, char **argv, int block_size, dim3 &dimsA, dim3 &dimsB)
{
    ...
    //    float *h_C = NULL;
    //    cudaHostAlloc((void**)&h_C,mem_size_C,0);
...
int main(int argc, char **argv)
{
  ...
  gettimeofday(&tv0,NULL);
    ...
    //    dim3 dimsA(4*50*block_size, 4*50*block_size, 1);
    //    dim3 dimsB(4*50*block_size, 4*50*block_size, 1);

    //normal
    int size = 460;
    dim3 dimsA(size*block_size, size*block_size, 1);
    dim3 dimsB(size*block_size, size*block_size, 1);
