...
typedef enum {
   ...
   * structure is \ref CUpti_ActivityKernel2.
   */
...
typedef enum {
   ...
   * Valid for CUpti_ActivityMetric.
   */
   ...
   * metric is missing.  Valid for CUpti_ActivityMetric.
   */
 ...
 * by CUPTI. Kernel activities are not reported using the
 * CUpti_ActivityKernel2 activity record.
 */
typedef struct PACKED_ALIGNMENT {
  /**
...
/**
 * \brief The activity record for a kernel (CUDA 5.5 onwards).
 *
 * This activity record represents a kernel execution
 * (CUPTI_ACTIVITY_KIND_KERNEL and
 * CUPTI_ACTIVITY_KIND_CONCURRENT_KERNEL).
 ...
 * information.
 * This activity record represents a CUPTI metric
 * value for a specific metric domain instance (CUPTI_ACTIVITY_KIND_METRIC_INSTANCE).
 * This activity record kind is not
 * produced by the activity API but is included for completeness and
 * ease-of-use. Profile frameworks built on top of CUPTI that collect
 * metric data may choose to use this type to store the collected metric
 * data. This activity record should be used when metric domain instance
 * information needs to be associated with the metric.
 */
typedef struct PACKED_ALIGNMENT {
  /**
   * The activity record kind, must be
...
typedef enum {
    /**
     * The device memory reserved for storing profiling data for
     * non-CDP operations for each stream. The value is a size_t.
     *
     * Larger buffers require less flush operations but consume more
     * device memory.  Small buffers might increase the risk of
     * missing timestamps for concurrent kernel records in the
     * asynchronous buffer handling mode if too many kernels are
     * launched/replayed between context synchronizations.  This value
     * only applies to new allocations.
     *
     * Set this value before initializing CUDA or before creating a
     * stream to ensure it is considered for the following
     * allocations.
     *
     * Note: The actual amount of device memory per stream reserved by
     * CUPTI might be larger.
     */
    CUPTI_ACTIVITY_ATTR_DEVICE_BUFFER_SIZE          = 0,
    /**
     * The device memory reserved for storing profiling data for CDP
     * operations for each stream. The value is a size_t.
     *
     * Larger buffers require less flush operations but consume more
     * device memory.  This value only applies to new allocations.
     *
     * Set this value before initializing CUDA or before creating a
     * stream to ensure it is considered for the following
     * allocations.
    ...
    /**
     * The maximum number of device memory buffers stored for reuse by
     * CUPTI. The value is a size_t.
     *
     * Buffers can be reused by streams of the same context.
     * Increasing this value reduces the profiling overhead when the
     * application creates and destroys many streams.  Setting this
 ...
 * \retval CUPTI_SUCCESS
 * \retval CUPTI_ERROR_NOT_INITIALIZED
 * \retval CUPTI_ERROR_INVALID_OPERATION if preceeded by a successful
 * call to cuptiActivityRegisterCallbacks
 * \retval CUPTI_ERROR_INVALID_PARAMETER if \p buffer is NULL, does
 * not have alignment of at least 8 bytes, or is not at least 1024
 * bytes in size
 *
 ...
 * cuptiActivityFlushAll() should be adopted.
 */
 ...
 * \retval CUPTI_ERROR_INVALID_OPERATION if preceeded by a successful
 * call to cuptiActivityRegisterCallbacks
 * \retval CUPTI_ERROR_INVALID_PARAMETER if \p buffer or \p
 * validBufferSizeBytes are NULL
 ...
 * buffer is associated with the global activities.
