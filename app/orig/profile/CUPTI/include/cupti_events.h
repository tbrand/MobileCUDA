...
typedef enum {
   ...
   * cuptiEventGroupEnable and cuptiEventGroupDisable calls. This is
   * the default mode.  For devices with compute capability less than
   * 2.0, event values are reset when a kernel is launched. For all
   * other devices event values are only reset when the events are
   * read.
   */
 ...
 * CUPTI_EVENT_COLLECTION_MODE_CONTINUOUS.  All previously enabled
